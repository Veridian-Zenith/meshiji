...
bool authenticate_user(const std::string &username, const std::string &password,
  ...
  if (ret != PAM_SUCCESS) {
    log_message(
        3, "PAM initialization failed: " + std::string(pam_strerror(pamh, ret)),
  ...
  if (ret != PAM_SUCCESS) {
    log_message(4,
                "PAM authentication failed for user " + username + ": " +
                    std::string(pam_strerror(pamh, ret)),
                config.log_file, false);
    pam_end(pamh, ret);
    return false;
  }
  ...
  if (ret != PAM_SUCCESS) {
    log_message(4,
                "PAM account management failed for user " + username + ": " +
                    std::string(pam_strerror(pamh, ret)),
                config.log_file, false);
    pam_end(pamh, ret);
    return false;
  ...
  log_message(6, "PAM authentication successful for user: " + username,
...
bool authenticate_and_escalate(const std::string &username,
  ...
  if (!check_permissions(username, config)) {
    log_message(4, "DENY user=" + username, config.log_file, false);
    std::cout
        << username
        << " not allowed. Add to /etc/voix/config.lua if this was intentional."
        << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < config.max_auth_attempts; ++i) {
    ...
    if (authenticate_user(username, password, config)) {
      if (setuid(0) != 0) {
        log_message(3, "SEUIDFAIL user=" + username, config.log_file, false);
  ...
  log_message(4, "AUTHFAIL user=" + username + " reason=max_attempts",
              config.log_file, false);
